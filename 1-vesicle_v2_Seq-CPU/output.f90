...
contains
  ...
  subroutine output_overlap
    ...
    write(21,'(I9,2ES10.2,2I6)') istep, h_avr/max(l_col,1),h_max,l_bad,l_col 
    close(21)

    !########debug##########
    if (istep > 20*1000 .and. l_bad > 0) then
       open( 17,file=trim(datadir)//'log.txt',position='append')
       write(17,'(A)') ''
       write(17,'(A)') 'Got a bad pair. Check the overlap.txt.'
       write(17,'(A)') 'Program terminated.'
       close(17)    
       stop
    endif
  ...
  subroutine output_stress(t)
    ...
    hydrodynamic: if (oscillatory_shear) then
       s_hyd(2,3) = s_hyd(2,3) + mu_f*(amp0*freq*cos(freq*t))*(1.+2.5*vol_frac) 
       s_hyd(3,2) = s_hyd(3,2) + mu_f*(amp0*freq*cos(freq*t))*(1.+2.5*vol_frac)
       
       if (rotating_shear) then
       	s_hyd(1,3) = s_hyd(1,3) + mu_f*(amp0*freq*sin(freq*t))*(1.+2.5*vol_frac) 
       	s_hyd(3,1) = s_hyd(3,1) + mu_f*(amp0*freq*sin(freq*t))*(1.+2.5*vol_frac)
       endif       
       
    else
       s_hyd(2,3) = s_hyd(2,3) + mu_f*shear_rate*(1.+2.5*vol_frac) 
       s_hyd(3,2) = s_hyd(3,2) + mu_f*shear_rate*(1.+2.5*vol_frac) 
    endif hydrodynamic

    ! bulk stress tensor (adding the per-particle hydro. stresslet)
    stress(:,:) = ((s_rep(:,:) + s_lub(:,:) + s_ctt(:,:))/vol_box) + s_hyd(:,:)
    ...
    ss = s_ctt/vol_box
    open( 23,file=trim(datadir)//'bulk_stress_ctt.txt',position='append')
    write(23,'(I10,9ES12.3)') istep, &
         ss(1,1),ss(1,2),ss(1,3), &
         ss(2,1),ss(2,2),ss(2,3), &
         ss(3,1),ss(3,2),ss(3,3)
    ...
    ss = s_hyd
    open( 23,file=trim(datadir)//'bulk_stress_hyd.txt',position='append')
    write(23,'(I10,9ES12.3)') istep, &
         ss(1,1),ss(1,2),ss(1,3), &
         ss(2,1),ss(2,2),ss(2,3), &
         ss(3,1),ss(3,2),ss(3,3)
  ...
  subroutine output_vtk(iter)
    ...
    !-- particle real positions --!
   
    !fh = 11 ! file handle
    
    !write(iterchar,'(i7.7)') iter/1000
    
    filename = trim(datadir)//'para'//trim(dsub_part)//'/para'//iterchar//'k.vtk'
    ...
    j=1
    do i = 1,np
       write(fh,'(3F12.6)') ppr(j),ppr(j+1),ppr(j+2) 
       j=j+3
    enddo
  ...
  subroutine output_restart(iter)
    ...
    write(iterchar,'(i7.7)') iter/1000
    filename = trim(datadir)//'restart_sim/restart'//iterchar//'k.txt'
    
    open(fh, file=filename, status='replace')

    write(fh,'(A)') 'This is a restarting file from istep, time (t9)'
    write(fh,'(I10)') istep
    write(fh,'(1ES16.8)') t9
    ...
    if (rotating_shear) then
        write(fh,'(2ES16.8)') x_shift, u_shift
    endif
    ...
    ! write the ppr data: particle's real positions
	j=1
    do i = 1,np
       write(fh,'(I6,3ES16.8)') i, ppr(j),ppr(j+1),ppr(j+2) 
       j=j+3
    enddo
    !----!
    ...
    filename = 'restart_500/restart_latest.txt'
    
    open(fh, file=filename, status='replace')

    write(fh,'(A)') 'This is a copy of restarting file from istep, time (t9)'
    write(fh,'(I10)') istep
    write(fh,'(1ES16.8)') t9
    write(fh,'(4ES16.8)') vol_frac, lx,ly,lz
    write(fh,'(2ES16.8)') y_shift, v_shift

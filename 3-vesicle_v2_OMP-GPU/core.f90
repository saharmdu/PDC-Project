...
contains
  ...
  subroutine time_marching(istart,iend)
    ...
    print *, 'simulation starts at istep = ', istart, ' at time = ', t9
    ...
    do istep = istart+1,iend
       ...
       !print *, "Calling integrator from time_marching"
       call integrator
       call data_output(t9)
    enddo
  ...
  subroutine init_marching
    ...
    print *, "Calling gather_nb_info from init_marching"
  ...
  subroutine integrator
    ...
    case(1) ! Forward Euler (not tested)
       ...
       ppr = ppr + dt*pv + (0.5*dt**2)*pa

       pv_new = pv + dt*pa
       pw_new = pw + dt*pb
       
       !print *, "Calling gather_nb_info from integrator-case1"
    ...
    case(2) ! (modified) Velocity-Verlet
       ...
       !print *, "Calling gather_nb_info from integrator-case2"
  ...
  subroutine gather_nb_info(pos,vel)      !Parallel Function
    ...
    !$OMP target teams distribute parallel do num_teams(128) thread_limit(128) &
    !$OMP& default(private) map(to: pos, vel, radii) &
    !$OMP& map(tofrom: nb_ind, num_nb, d_ij_mx, r_ij_mx, v_ij_mx) &
    !$OMP& reduction(+: l_col, l_bad, h_avr, h_max)
  ...
  subroutine comp_force_torque(pos,vel,ori,omg, acc,bta)    !Parallel Function
    ...
    !$OMP target teams distribute parallel do num_teams(128) thread_limit(128) &
    !$OMP& default(private) map(to: pos, vel, omg, radii, nb_ind, num_nb, r_ij_mx, v_ij_mx) &
    !$OMP& map(tofrom: acc, bta)
    ...
    do i = 1,np
       ...
       !edl_interaction: if (elst_repl) then
       if (elst_repl) then
          do j = i+1,np  ! all the  higher-index j>i
            ...
            call comp_elst_vdw(i,j,a_i,a_j,acc)
            
          enddo 
       end if !edl_interaction
  ...
  subroutine comp_Stokes(i,pos_i,vel_i,omg_i, acc,bta)
    ...
    if (oscillatory_shear) then
       ...
       if (rotating_shear) then
          vel_infty(1) = (pos_i(3)-lz/2.)*(amp0*lz*freq/lz)*cos(freq*t9 -pi/2.)
       endif
    else
    ...
    if (oscillatory_shear) then
       ...
       if (rotating_shear) then
          omg_infty(2) = (amp0*lz*freq/lz)*cos(freq*t9 -pi/2.)/2.
       endif
    else
  ...
  subroutine comp_elst_vdw(i,j,a_i,a_j,acc)
    ...
    F_rep_mag = -F_elst*(buf0/a_1)*exp(-max(h_ij,0.)/Debye_len)
  ...
  subroutine comp_least_dist(a_i,a_j,pos_i,pos_j,vel_j,d_ij)
    ...
    real, dimension(7) :: xj_im,yj_im,zj_im, uj_im_shift,vj_im_shift, d_im
    ...
    if (d_ij > a_i + a_j + h_lub_out) then
       ...
       if (zi < drim) then
          ...
          if (rotating_shear) then
             xj_im(4) = mod(lx+xj-x_shift, lx)
             uj_im_shift(4:7) = -u_shift
          else
             xj_im(4) = xj
          endif
    ...
    if (jj /= 0) then
       ...
       pos_j(3) = zj_im(jj)
       if (rotating_shear) then
          vel_j(1) = vel_j(1) + uj_im_shift(jj)
       endif
  ...
  subroutine periodicity
    ...
    if (oscillatory_shear) then
       ...
       if (rotating_shear) then
          x_shift = mod(amp0*lz*sin(freq*t9 -pi/2.), lx) ! bounded in [0,lx)
